...
object Api {
  ...
  final case class Live(idProvider: IdProvider) extends Api {
    override def createBlog(name: Blog.Name, posts: List[(Option[Post.Title], Post.Body)]): UIO[(Blog.Id, List[Post.Id])] = {
      for {
        blogId <- idProvider.generateId.map(Blog.Id(_))
        postIds <- ZIO.replicateM(posts.length)(idProvider.generateId).map(_.map(Post.Id(_)).toList)
  ...
  def createBlog(name: Blog.Name, posts: List[(Option[Post.Title], Post.Body)]): URIO[Has[Api], (Blog.Id, List[Post.Id])] =
